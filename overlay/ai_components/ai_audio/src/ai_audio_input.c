...
OPERATE_RET ai_audio_input_start(void)
{
    ...
    if (!sg_recorder->vad_task) {
        THREAD_CFG_T thrd_cfg = {
            ...
            /* DuckyClaw overlay: keep record_task stack in INTERNAL SRAM
             * regardless of ENABLE_EXT_RAM. This task polls AFE VAD every
             * 10 ms; PSRAM stack adds enough access latency to break the
             * WebRTC VAD timing inside AFE and wake-word detection stops
             * firing on this board (raw I2S no-codec mic path is timing
             * sensitive). Other DuckyClaw threads still use PSRAM where
             * they request .psram_mode = 1. */
            .psram_mode = 0,
        };
